-- Defense in depth: enforce attention session freshness inside the redeem RPC too.
-- Edge Functions already verify expires_at, but the RPC should fail closed if called directly.
...
BEGIN
  ...
  SELECT id, user_id, campaign_id, validated, redeemed_at, expires_at, validation_score, reward_multiplier
  INTO v_session
...
COMMENT ON FUNCTION public.redeem_attention_reward(UUID, UUID, INTEGER, INTEGER, INTEGER)
  IS 'Redeem promo attention session: locks attention_sessions + daily_reward_caps FOR UPDATE, rejects expired sessions, checks caps, ledger_append, updates cap and session.';
