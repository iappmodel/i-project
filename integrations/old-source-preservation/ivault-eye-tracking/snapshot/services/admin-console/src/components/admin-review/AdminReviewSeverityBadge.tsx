import { REVIEW_SEVERITY_LABELS } from "@/lib/admin-review/admin-review-ui-rules";

export function AdminReviewSeverityBadge(props: { severity?: string | null }) {
  ...
  return (
    <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${tone}`}>
      {REVIEW_SEVERITY_LABELS[severity] ?? severity}
    </span>
  );
}
