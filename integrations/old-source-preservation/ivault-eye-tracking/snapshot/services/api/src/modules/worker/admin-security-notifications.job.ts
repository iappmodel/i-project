...
async function sendSlack(input: {
  ...
  const severity =
    typeof payload.severity === "string" ? payload.severity.toUpperCase() : "SECURITY";
  const eventKey =
    typeof payload.event_key === "string"
      ? payload.event_key
      : typeof payload.alert_key === "string"
        ? payload.alert_key
...
async function sendEmailStub(input: {
  ...
  // Stub only. Replace with provider adapter when wiring real email sending.
...
export async function runAdminSecurityNotificationDeliveryJob() {
  ...
  const items = (data as ClaimedDelivery[] | null) ?? [];
