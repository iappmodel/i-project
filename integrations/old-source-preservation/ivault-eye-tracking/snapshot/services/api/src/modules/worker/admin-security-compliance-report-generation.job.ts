...
function renderMarkdown(input: { job: any; sections: any[]; evidence: any[] }) {
  const lines: string[] = [];
...
function renderJson(input: { job: any; contentSummary: any; sections: any[]; evidence: any[] }) {
  ...
  for (const job of claimed as Record<string, any>[]) {
    try {
      ...
      if (job.report_format === "json") {
        body = renderJson({ job, contentSummary, sections, evidence });
        ...
        body = JSON.stringify(
          {
            ...
            markdown: renderMarkdown({ job, sections, evidence }),
            manifest: {
        ...
        body = renderMarkdown({ job, sections, evidence });
      }
      ...
      const signedBody =
        job.report_format === "markdown"
          ? [
              ...
              `- Report checksum: \`${unsignedChecksum}\``,
              `- Signature: \`${signature}\``
            ].join("\n")
          : body;
      ...
      const { storageUri } = await writeReportFile({ job, body: signedBody });
      ...
      const message =
        err instanceof Error ? err.message : "unknown compliance report generation error";
