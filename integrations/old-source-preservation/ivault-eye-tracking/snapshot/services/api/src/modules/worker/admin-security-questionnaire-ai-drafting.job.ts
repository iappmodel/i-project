...
function buildDraftFromAnswer(input: { answer: any | null; evidence: any[] }) {
  if (input.answer) {
...
function checkGuardrails(input: { generatedAnswer: string; evidenceCount: number; guardrails: any[] }) {
  ...
  for (const guardrail of input.guardrails) {
    ...
    if (!re.test(input.generatedAnswer)) continue;
    const isUnsupportedEvidenceClaim =
      guardrail.guardrail_type === "unverified_evidence" && input.evidenceCount === 0;
    const shouldFlag = guardrail.guardrail_type !== "unverified_evidence" || isUnsupportedEvidenceClaim;
...
export async function runAdminSecurityQuestionnaireAiDraftingJob() {
  ...
  const { data: jobs, error: claimError } = await supabaseAdmin.rpc(
    "claim_admin_security_questionnaire_ai_drafts",
    {
      ...
      p_metadata: { source: "admin-security-questionnaire-ai-drafting-worker" }
    }
  );
  ...
  for (const job of claimed) {
    try {
      ...
      const scored = answers
        .map((answer) => ({ answer, ...scoreAnswer(job, answer) }))
      ...
      if (job.draft_mode !== "match_only") {
        const draft = buildDraftFromAnswer({ answer: selectedAnswer, evidence });
      ...
      const safety = generatedAnswer
        ? checkGuardrails({ generatedAnswer, evidenceCount: evidence.length, guardrails })
        : { safetyStatus: "passed", safetyFlags: [] };
      ...
      const { error: completeError } = await supabaseAdmin.rpc(
        "complete_admin_security_questionnaire_ai_draft",
        {
          ...
          p_worker_id: workerId,
          p_metadata: { workerId, source: "admin-security-questionnaire-ai-drafting-worker" }
        }
      );
