-- 49/post-MVP schema — audit hash chain tamper evidence and verification.
...
create or replace function audit_sha256_text(
  p_input text
)
returns text
language sql
...
create or replace function hash_accounting_journal_entry(
  p_accounting_journal_entry_id uuid,
  p_metadata jsonb default '{}'::jsonb
)
returns uuid
language plpgsql
...
begin
  ...
  where id = p_accounting_journal_entry_id;

  if v_journal.id is null then
    raise exception 'accounting journal entry not found: %',
      p_accounting_journal_entry_id;
  end if;
...
begin
  ...
  where id = p_admin_audit_log_id;

  if v_row.id is null then
    raise exception 'admin audit log entry not found: %',
      p_admin_audit_log_id;
  end if;
...
begin
  select *
  into v_row
  from trust_score_override_events
  where id = p_trust_score_override_event_id;

  if v_row.id is null then
    raise exception 'trust score override event not found: %',
      p_trust_score_override_event_id;
  end if;

  return append_audit_hash_chain_entry(
    'trust_score_override_event',
...
begin
  ...
  if v_event.id is null then
    raise exception 'attention verification event not found: %',
  ...
  v_payload := jsonb_build_object(
    ...
    to_jsonb(v_event),
    'fraud_signals',
    coalesce(v_fraud_signals, '[]'::jsonb)
  );
...
begin
  ...
  if v_row.id is null then
    raise exception 'reward issuance group not found: %',
  ...
  return append_audit_hash_chain_entry(
    ...
    v_row.id,
    to_jsonb(v_row),
    coalesce(v_row.completed_at, v_row.created_at),
    'global_audit_chain',
    p_metadata
  );
end;
...
select
  'wallet_ledger_entry' as event_type,
  'wallet_ledger_entries' as event_table,
  wle.id as event_id,
  wle.created_at as event_occurred_at
from wallet_ledger_entries wle
left join audit_hash_chain_entries h
  on h.event_table = 'wallet_ledger_entries'
 and h.event_id = wle.id
where h.id is null
...
select
  ...
  'accounting_journal_entries',
  aje.id,
  coalesce(aje.posted_at, aje.created_at)
from accounting_journal_entries aje
left join audit_hash_chain_entries h
  on h.event_table = 'accounting_journal_entries'
 and h.event_id = aje.id
...
left join audit_hash_chain_entries h
  on h.event_table = 'admin_audit_log'
 and h.event_id = aal.id
where h.id is null
...
left join audit_hash_chain_entries h
  on h.event_table = 'trust_score_override_events'
 and h.event_id = tsoe.id
where h.id is null
...
left join audit_hash_chain_entries h
  on h.event_table = 'attention_verification_events'
 and h.event_id = ave.id
where h.id is null
...
select
  ...
  'reward_issuance_groups',
  rig.id,
  coalesce(rig.completed_at, rig.created_at)
from reward_issuance_groups rig
left join audit_hash_chain_entries h
  on h.event_table = 'reward_issuance_groups'
 and h.event_id = rig.id
...
begin
  ...
  for v_event in
    select *
    from audit_hash_missing_events
      ...
      if v_event.event_table = 'wallet_ledger_entries' then
        perform hash_wallet_ledger_entry(
          ...
          p_metadata || jsonb_build_object(
            'audit_hash_backfill_run_id',
            v_run_id
          )
        );

      elsif v_event.event_table = 'accounting_journal_entries' then
        ...
        perform hash_admin_audit_log_entry(
          v_event.event_id,
          p_metadata || jsonb_build_object(
            'audit_hash_backfill_run_id',
            v_run_id
        ...
        perform hash_trust_score_override_event(
          v_event.event_id,
          p_metadata || jsonb_build_object(
            'audit_hash_backfill_run_id',
            v_run_id
...
begin
  ...
  insert into audit_hash_chain_anchors (
    ...
    anchor_type,
    anchor_uri,
    anchored_by_admin_id,
    metadata
  )
  values (
    p_chain_key,
    v_latest.sequence_number,
    v_latest.chain_hash,
    p_anchor_type,
    p_anchor_uri,
    p_anchored_by_admin_id,
    p_metadata
  )
