...
class IntentEngine {
  ...
  void update(RuntimeSignals s) {
    if (s.confidence < _intentConfidenceThreshold) return;
  ...
  void updateFromAttentionState(AttentionState state) {
    update(RuntimeSignals(
      state: state,
  ...
  void updateDwell(RuntimeSignals s) {
    if (!s.state.isFixating) return;
    _updateDwell(s.state);
  }
