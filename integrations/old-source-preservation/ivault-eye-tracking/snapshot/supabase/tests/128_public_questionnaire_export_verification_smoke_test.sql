...
begin
  v_admin_user_id := upsert_admin_user(
    ...
    'questionnaire-export-verification-admin@example.com',
    'Questionnaire Export Verification Admin',
    'active',
    '{"test": true}'::jsonb
  );

  perform assign_admin_role(v_admin_auth_user_id, 'super_admin', null, 'questionnaire export verification bootstrap');
  ...
  v_project_id := create_admin_security_questionnaire_project(
    ...
    null,
    null,
    'verify-questionnaire-project-create',
    '{"test": true}'::jsonb
  );
  ...
  values (
    ...
    false,
    now(),
    'file:///tmp/verify-questionnaire-export.json',
    repeat('a', 64),
    1000,
    1,
    0,
    ...
    repeat('b', 64),
    now(),
    'QUESTIONNAIRE_EXPORT=verify-questionnaire-export',
  ...
  if (v_result->>'verificationStatus') <> 'verified' then
    raise exception 'expected verified questionnaire export, got %', v_result;
  end if;
end $$;
