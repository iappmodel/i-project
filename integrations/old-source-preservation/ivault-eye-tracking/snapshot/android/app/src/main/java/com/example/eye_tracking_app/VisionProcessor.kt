...
object VisionProcessor {
    ...
    /**
     * Run [ImageSegmenter.segment] every Nth face frame; reuse cached mask metrics in between.
     * Segmentation is a large fraction of native time vs face mesh alone.
     */
    ...
    @Volatile
    private var lastSelfieQualityCached: Float = -1f

    @Volatile
    private var lastFaceConfidenceCached: Double = -1.0
    ...
    /**
     * Segmenter quality score placeholder.
     *
     * Current MediaPipe Tasks version in this project does not expose `qualityScores()` on
     * `ImageSegmenterResult`, so keep returning `-1f` while preserving the `selfieQuality` output key.
     */
     ...
     */
    /**
     * Approximate person fill on the category mask by grid subsampling (stride in pixels).
     * Full raster scans were ~O(2.5M) per frame on 1080×class previews and dominated native time.
     */
    private fun categoryMaskPersonFillRatio(
        ...
        return try {
            ...
            val s = gridStride.coerceAtLeast(1)
            var personPixels = 0
            var sampled = 0
            var y = 0
            while (y < h) {
                ...
                val rowOff = y * w
                while (x < w) {
                    val v = buf.get(rowOff + x).toInt() and 0xFF
            ...
            personPixels.toDouble() / sampled.toDouble()
        } finally {
            mask.close()
        }
    }
    ...
    fun process(bitmap: Bitmap): Map<String, Any> {
        ...
        return try {
            ...
            val (selfieQuality, faceConfidence) = try {
                ...
                if (seg != null) {
                    val n = ++segmentFrameCounter
                    if (n % SEGMENT_EVERY_N_FRAMES == 0) {
                        val segResult = seg.segment(mpImage)
                        val sq = selfieQualityMean(segResult)
                        val fc = categoryMaskPersonFillRatio(segResult)
                        lastSelfieQualityCached = sq
                        lastFaceConfidenceCached = fc
                        Pair(sq, fc)
                    } else {
                        Pair(lastSelfieQualityCached, lastFaceConfidenceCached)
                    }
                } else {
                    Pair(-1f, -1.0)
                }
