...
export function WalletScreen() {
  const { walletBalance, pendingBalance, aCoins, iCoins, transactions, setScreen } = useDemo()
  ...
  return (
    <PhoneFrame scroll>
      ...
      <p className="balance-sub-wallet-ui">
        Liquid rails · <span>icoins weighted</span>
      </p>
      ...
      <div className="action-row-wallet-ui">
        ...
        <button type="button" className="act-btn-wu" aria-disabled>
          <span className="act-icon-wu" style={{ opacity: 0.4 }}>
            ◎
          </span>
      ...
      <div className="section-hdr-wu">
        ...
        <button type="button" className="sec-link-wu">
          See all
        </button>
      </div>
      ...
      <div className="activity-stack-wu">
        {transactions.slice(0, 5).map((t) => (
          <button
            key={t.id}
            type="button"
            className={`activity-card-wu ${t.kind}`}
          >
            ...
            <span className={`act-amount-wu mono ${t.kind === 'positive' ? 'positive' : t.kind === 'negative' ? 'negative' : 'pending'}`}>
              {t.amountDisplay}
            </span>
          </button>
        ))}
      ...
      <button type="button" className="cashout-cta prot-cta-flat" style={{ marginTop: 16 }} onClick={() => setScreen('withdraw-preview')}>
