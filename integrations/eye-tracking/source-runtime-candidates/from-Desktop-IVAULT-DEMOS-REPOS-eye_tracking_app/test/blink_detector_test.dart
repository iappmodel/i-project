...
void main() {
  ...
  test('meanEarIsBlinking false when hasFace is false', () {
    expect(
      meanEarIsBlinking(
        wasBlinking: false,
        avgEar: 0.04,
        hasFace: false,
        closeThreshold: BlinkDetector.EAR_CLOSED_THRESHOLD,
        openThreshold: BlinkDetector.rawOpenThreshold,
      ),
      false,
    );
  });

  test('meanEarIsBlinking false when avgEar <= 0', () {
    expect(
      meanEarIsBlinking(
        wasBlinking: false,
        avgEar: 0,
  ...
  test('meanEarIsBlinking false when not wasBlinking and avgEar == closeThreshold', () {
    final c = BlinkDetector.EAR_CLOSED_THRESHOLD;
    expect(
      meanEarIsBlinking(
        wasBlinking: false,
        avgEar: c,
  ...
  test('meanEarIsBlinking true when not wasBlinking and avgEar < closeThreshold', () {
    final c = BlinkDetector.EAR_CLOSED_THRESHOLD;
    expect(
      meanEarIsBlinking(
        wasBlinking: false,
  ...
  test('meanEarIsBlinking true when wasBlinking and avgEar < openThreshold', () {
    final c = BlinkDetector.EAR_CLOSED_THRESHOLD;
    final o = BlinkDetector.rawOpenThreshold;
    expect(
      meanEarIsBlinking(
        wasBlinking: true,
        avgEar: o - 1e-6,
  ...
  test('meanEarIsBlinking false when wasBlinking and avgEar == openThreshold', () {
    final c = BlinkDetector.EAR_CLOSED_THRESHOLD;
    final o = BlinkDetector.rawOpenThreshold;
    expect(
      meanEarIsBlinking(
  ...
  test('meanEarFromPairIfFinite null left returns null', () {
    expect(
      meanEarFromPairIfFinite(leftEar: null, rightEar: 0.25),
      isNull,
    );
  });

  test('meanEarFromPairIfFinite null right returns null', () {
    expect(
      meanEarFromPairIfFinite(leftEar: 0.25, rightEar: null),
      isNull,
    );
  });

  test('meanEarFromPairIfFinite NaN returns null', () {
    expect(
      meanEarFromPairIfFinite(leftEar: 1.0, rightEar: double.nan),
      isNull,
    );
    expect(
      meanEarFromPairIfFinite(leftEar: double.nan, rightEar: 1.0),
      isNull,
    );
  });

  test('meanEarFromPairIfFinite infinity returns null', () {
    expect(
      meanEarFromPairIfFinite(leftEar: 1.0, rightEar: double.infinity),
      isNull,
    );
    expect(
      meanEarFromPairIfFinite(leftEar: double.negativeInfinity, rightEar: 1.0),
      isNull,
    );
  });

  test('meanEarFromPairIfFinite finite pair returns arithmetic mean', () {
    expect(
      meanEarFromPairIfFinite(leftEar: 0.2, rightEar: 0.4),
      closeTo(0.3, 1e-12),
    );
  });

  test('meanEarFromPairIfFinite negative finite values average normally', () {
    expect(
      meanEarFromPairIfFinite(leftEar: -2.0, rightEar: -4.0),
      closeTo(-3.0, 1e-12),
    );
  });
}
